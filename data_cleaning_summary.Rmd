...
```{r}
# Assuming merged_data has the state information in a column named 'state'
# and the total population for each flood event in a column named 'totalPopulation'

# Calculate the number of floods per state
floods_by_state <- merged_data %>%
  count(state, name = "number_of_floods")
# Analyze the relationship with a scatter plot
ggplot(state_summary, aes(x = Mean_Total_Population, y = number_of_floods)) +
  geom_point() +
